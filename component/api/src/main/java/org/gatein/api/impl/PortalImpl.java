...
import org.gatein.api.impl.portal.navigation.LoadedNodeScope;
...
import org.gatein.api.impl.portal.navigation.NodePathScope;
...
public class PortalImpl extends DataStorageContext implements Portal, Startable
{
   ...
   public void saveNavigation(Navigation navigation)
   {
      ...
      {
         NavigationServiceContext ctx = new NavigationServiceContext(navigationService, navigation.getSiteId(),
               new LoadedNodeScope());
         ctx.updateNavigation(navigation);
      }
   }
   ...
   /**
    * TODO Cascade save of children
    * 
    * TODO Deal with consistency issues (look at NavigationImporter)
    */
   @Override
   public void saveNode(Node node)
   {
      // synchronized (node)
      // {
      // NavigationContext navCtx = getNavigationContext(node.getPageId().getSiteId(), true);
      // NodeContext<NodeContext<?>> rootNodeCtx = getRootNodeContext(new LoadedNodeScope(), navCtx);
      // NodeContext<NodeContext<?>> nodeCtx = NavigationUtil.findNodeContext(rootNodeCtx, node.getPath());
      //
      // if (nodeCtx == null)
      // {
      // NodeContext<NodeContext<?>> parentNodeCtx = NavigationUtil.findNodeContext(rootNodeCtx, node.getPath().getParent());
